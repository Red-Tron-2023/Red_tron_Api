...
const getAll = async (req: Request, res: Response, next: NextFunction) => {
  const {adminId, tellerId, casinoId, userCasinoId, date, end } = req.query
  // agregar busquedas por rangos de montos
  return response(res, StatusCodes.OK,await controller.getAll(adminId as string, tellerId as string, casinoId as string, userCasinoId as string, date as string, end as string));
}
